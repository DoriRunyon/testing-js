...
describe("Tic Tac JS", function () {
    ...
    it("should update the board correctly based on the board", function () {
        ...
        board = ["X", null, null, null, null, null, null, null, null];
        updateBoard();
        var cellZeroText = $("#cell-0").text();
        expect(cellZeroText).toEqual("X");
    });

    it("should not fill in spots that are already occupied", function () {

        board = ["X", null, null, null, null, null, null, null, null];
        overlap = placePiece(0, "X");
        notOverlap = placePiece(1, "X");
        expect(overlap).toBeFalsy();
        expect(notOverlap).toBeTruthy();
